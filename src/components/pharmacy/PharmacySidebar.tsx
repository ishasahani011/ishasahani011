...
export const PharmacySidebar: React.FC<PharmacySidebarProps> = ({
  ...
  return (
    <aside className="w-64 bg-[#eff4ff] border-r border-[#dce9ff] flex flex-col justify-between shrink-0 select-none">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all ${
                activeTab === 'auth'
                  ...
                  : 'text-[#006a61] bg-[#dce9ff]/60 hover:bg-[#dce9ff]'
              }`}
            >
              <div className="flex items-center gap-2">
                <KeyRound className="w-4 h-4" />
                <span className="font-bold">Staff Login &amp; Station Reg</span>
              </div>
              <span className="text-[9px] bg-[#86f2e4] text-[#006f66] font-mono px-1.5 py-0.5 rounded font-bold">
                AUTH
              </span>
            </button>
          </div>
      ...
      {/* Authenticated Staff Badge & DEA Vault Status */}
      <div className="p-3 bg-white/90 border-t border-[#dce9ff] flex flex-col gap-2">
        {pharmacyUser ? (
          <div 
            onClick={() => onTabChange('auth')}
            className="p-2 rounded-xl bg-[#eff4ff] border border-[#dce9ff] hover:border-[#006a61] cursor-pointer transition-all space-y-1"
            title="Click to manage staff credentials or station registration"
          >
            <div className="flex items-center justify-between">
              <span className="text-[9px] uppercase font-bold text-[#73777d] tracking-wider">Active Staff Terminal</span>
              <span className="px-1.5 py-0.2 rounded bg-[#006a61] text-white text-[9px] font-mono font-bold">
                {pharmacyUser.role}
              </span>
            </div>
            <p className="font-bold text-[#001729] text-xs truncate">{pharmacyUser.fullName}</p>
            <div className="flex items-center justify-between text-[10px] text-[#006a61] font-mono">
              <span>{pharmacyUser.licenseNumber}</span>
              <span className="text-[#006f66] font-bold">CSOS ✓</span>
            </div>
          </div>
        ) : (
          <button
            onClick={() => onTabChange('auth')}
            className="w-full py-2 bg-[#006a61] text-white text-xs font-bold rounded-xl shadow-xs hover:bg-[#005049] transition-all flex items-center justify-center gap-1.5"
          >
        ...
        <div className="flex items-center justify-between text-xs font-semibold text-[#43474d] px-1 pt-1 border-t border-[#dce9ff]/60">
          <span className="flex items-center gap-1 text-[11px]">
            <Lock className="w-3 h-3 text-[#006a61]" />
            DEA 222 Vault
          </span>
          <span className="font-mono text-[10px] text-[#006a61]">ARMED</span>
        </div>
      </div>
    </aside>
  );
