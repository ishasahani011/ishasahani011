...
import { OpsAuthScreen } from './OpsAuthScreen';
import { useAuth } from '../../context/AuthContext';

export const MissionControl: React.FC = () => {
  const { opsUser } = useAuth();
  const [activeTab, setActiveTab] = useState<'dispatch' | 'compliance' | 'auth'>('compliance');
      ...
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        ...
        <div className="bg-white rounded-2xl p-5 border border-[#e5eeff] shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {/* Operator Session Indicator */}
            {opsUser ? (
              <button
                onClick={() => setActiveTab('auth')}
                className="px-3 py-1.5 rounded-xl bg-[#eff4ff] hover:bg-[#dce9ff] border border-[#dce9ff] text-xs flex items-center gap-2 text-left transition-all"
                title="Click to view Operator Profile / Switch Clearance"
              >
                <span className="w-2 h-2 rounded-full bg-[#006a61]"></span>
                <span className="font-bold text-[#001729] truncate max-w-[120px]">{opsUser.fullName}</span>
                <span className="px-1.5 py-0.2 rounded bg-[#006a61] text-white text-[9px] font-mono font-bold">
                  {opsUser.clearanceTier}
                </span>
              </button>
            ) : (
              <button
                onClick={() => setActiveTab('auth')}
                className="px-3 py-1.5 rounded-xl bg-[#006a61] hover:bg-[#005049] text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-xs"
              >
        ...
        {/* Sub-Navigation Switch: Compliance Center vs Live Dispatch vs Operator Auth */}
        <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-2 rounded-2xl border border-[#e5eeff] shadow-xs">
          <div className="flex flex-wrap items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
                activeTab === 'compliance'
                  ...
                  : 'bg-transparent text-[#43474d] hover:bg-[#eff4ff]'
              }`}
            >
              <HeartPulse className={`w-4 h-4 ${activeTab === 'compliance' ? 'text-[#86f2e4]' : 'text-[#006a61]'}`} />
              <span>Compliance Center (Patient Adherence D3 Analytics)</span>
              <span className={`text-[10px] font-mono font-bold px-1.5 py-0.2 rounded ${
                activeTab === 'compliance' ? 'bg-[#86f2e4] text-[#006f66]' : 'bg-[#eff4ff] text-[#006a61]'
              }`}>
                CMS 5-STAR
              </span>
            </button>
            ...
            >
              <Truck className={`w-4 h-4 ${activeTab === 'dispatch' ? 'text-[#86f2e4]' : 'text-[#73777d]'}`} />
              <span>Live Fleet Dispatch &amp; Hub Stations</span>
            </button>
            ...
            >
              <KeyRound className={`w-4 h-4 ${activeTab === 'auth' ? 'text-[#86f2e4]' : 'text-[#73777d]'}`} />
              <span>Operator &amp; Auditor Access</span>
              <span className="text-[10px] font-mono font-bold px-1.5 py-0.2 rounded bg-[#eff4ff] text-[#006a61]">
                RBAC
              </span>
            </button>
          </div>

          <div className="hidden sm:flex items-center gap-2 px-3 text-xs text-[#73777d]">
            <span className="w-2 h-2 rounded-full bg-[#006a61] animate-pulse"></span>
            <span className="font-mono text-[11px]">Real-Time Telemetry: Active</span>
          </div>
        </div>

        {/* Tab Content */}
        {activeTab === 'auth' ? (
          <OpsAuthScreen onSuccess={() => setActiveTab('compliance')} />
        ) : activeTab === 'compliance' ? (
          <ComplianceCenter />
        ) : (
          <>
